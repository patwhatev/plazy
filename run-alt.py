...
try:
	...
	for pick in pref_array:
		...
		# update formatting to include whitespace
		pick_formatted = quote(pick)

		# take pick, search the music map api
		url = 'http://www.similar-artist.info/similarto/artist/' + pick_formatted
		...
		found = soup.select_one("#listing .discounted-item")
		artist_arr = []
		if found:
			artists = soup.select("#listing .discounted-item h1")
			...
			for artist in artist_arr:
				...
				if tracks:
					for track in tracks:
						...
						# account for special characters when matching
						if unidecode(lowered_found_artist) == unidecode(lowered_artist):
							track_id = track['uri']
